...
export default async function PlantManagerDashboard() {
    ...
    if (reqError) {
        console.error("Plant Manager fetch error:", reqError)
    }
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-4 md:p-8">
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{_t.plantManager.title}</h1>
                        <p className="text-sm sm:text-base text-zinc-500">
                            {_t.plantManager.subtitle}
                        </p>
                    </div>
